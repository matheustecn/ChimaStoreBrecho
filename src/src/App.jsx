...
export default function ChimaStoreERP() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error || !data) {
        // Auth account exists but has no store profile (e.g. access was revoked) — sign out.
...
function SignUpForm({ notify }) {
  ...
  const criar = async () => {
    ...
    setLoading(true);
    const { data, error } = await supabase.auth.signUp({ email: normEmail(email), password: senha });
    if (error) {
      setLoading(false);
    ...
    // First person ever to sign up becomes the store administrator; everyone after is "caixa"
    // and can be promoted later in Configurações → Usuárias.
    const { count } = await supabase.from("profiles").select("id", { count: "exact", head: true });
    const role = (count || 0) === 0 ? "admin" : "caixa";
    const { error: profileError } = await supabase.from("profiles").insert({ id: data.user.id, name: name.trim(), role });
    setLoading(false);
    if (profileError) { notify("Conta criada, mas houve um erro ao configurar o perfil.", "error"); return; }
